...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MatButtonModule, MatIconModule, MatSnackBarModule, MailbagSummaryComponent],
...
export class ScanningWorkspaceComponent {
  ...
  scan() {
    ...
    this.api.scanParcel(this.sessionId(), code).subscribe({
      next: () => {
        ...
        this.error.set(null);
        // refresh session
        this.api.getSession(this.sessionId()).subscribe((s) => this.session.set(s));
      },
      error: (err) => {
        this.error.set(err?.error?.message || 'Failed to scan');
        this._snackBar.open(this.error()!, 'Close', { duration: 3000, verticalPosition: 'top' });
      },
    });
  }
  ...
  closeSession() {
    if (!this.session()) {
      ...
      this.error.set(msg);
      this._snackBar.open(msg, 'Close', { duration: 3000, verticalPosition: 'top' });
      return;
    }
    this.api.closeSession(this.sessionId()).subscribe({
      next: () => this.router.navigate(['/secure/scanning/session', this.sessionId(), 'delivery-note']),
      error: (err) => {
