...
export async function generateReceiptsForParcel(
  ...
  // Helper to format cargo type for display
  const formatCargoType = (cargoType: string) => {
    switch (cargoType) {
      ...
      case "FRAGILE":
        return "Fragile";
      case "ELECTRONIC_SENSITIVE":
        return "Electronic/Sensitive";
      case "DOCUMENT":
        return "Document";
      default:
        return cargoType;
  ...
  async function drawCenteredBarcode(
    doc: any,
    barcodePath: string,
    width: number
  ) {
  ...
  function drawParcelSummary(doc: any) {
    ...
    doc.font("Helvetica");
    doc.text(`Category Type: ${formatCargoType(parcel.cargoType)}`);
    doc.text(`Description: ${parcel.description}`);
    doc.text(
      `Declared Value: ZMW ${formatAmt(parcel.value as unknown as number)}`
    );
    doc.moveDown(0.3);
  }
  ...
  function drawReceiptLogo(doc: any, typeKey: string) {
    ...
    try {
      if (logoAsset.type === "svg") {
        ...
        const height =
          img && img.height ? img.height * scale : targetWidth * 0.6;
  ...
  for (const t of types) {
    ...
    if (showFullDetails) {
      ...
      if (parcel.payment) {
        ...
        if ((parcel.payment as any).reference)
          doc.text(`Ref: ${(parcel.payment as any).reference}`);
      }
      doc.moveDown(0.6);
    }
    ...
    const originName =
      (parcel as any).sendingOffice?.name || parcel.office.name;
    const originCode =
      (parcel as any).sendingOffice?.branchCode || parcel.office.branchCode;
    doc.text(`Office: ${originName} (${originCode})`);
    doc.text(`Date: ${formattedDate}`);
    const senderContact =
      normalizeZMBPhone((parcel as any).customer?.phoneNumber) ?? "";
    ...
    const receiverContact =
      normalizeZMBPhone((parcel as any).receiver?.phoneNumber) ?? "";
    doc.text(`Contact No: ${receiverContact}`);

    if (showFullDetails) {
      drawParcelSummary(doc);
    } else {
      // For short sticker version, still show cargo type
      doc.moveDown(0.5);
      doc.font("Helvetica-Bold").text("Parcel Details");
      doc.font("Helvetica");
      doc.text(`Category Type: ${formatCargoType(parcel.cargoType)}`);
