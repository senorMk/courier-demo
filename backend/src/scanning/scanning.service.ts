...
export class ScanningService {
  ...
  async startSession(userId: string, officeId: string, routeId: string, mode: 'bag' | 'individual') {
    ...
    if (office.routeId !== routeId) throw new BadRequestException('Office not on selected route');
  ...
  async scanParcel(sessionId: string, parcelId: string, userId: string) {
    ...
    const parcel = await this.prisma.parcel.findUnique({ where: { id: parcelId }, include: { office: true } });
